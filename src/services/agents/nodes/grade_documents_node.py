...
async def ainvoke_grade_documents_step(
    ...
    question, context = get_latest_context_and_query(state["messages"])
    ...
    try:
        ...
        logger.info(
            f"LLM grading: score={grading_response.binary_score}, reasoning={grading_response.reasoning}"
        )
    ...
    logger.info(
        f"Grading result: {'relevant' if is_relevant else 'not relevant'}, routing to: {route}"
    )
