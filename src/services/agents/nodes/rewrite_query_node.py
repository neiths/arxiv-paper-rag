...
async def ainvoke_rewrite_query_step(
    ...
    try:
        ...
        logger.debug(
            f"Invoking LLM for query rewriting (model: {runtime.context.model_name})"
        )
        llm_start = time.time()
        ...
        if not result or not result.rewritten_query:
            raise ValueError(
                "LLM failed to return valid structured output for query rewriting"
            )
            ...
            runtime.context.langfuse_tracer.update_span(
                ...
                metadata={
                    ...
                    "llm_duration_seconds": (
                        llm_duration if "llm_duration" in locals() else None
                    ),
                },
            )
        finally:
